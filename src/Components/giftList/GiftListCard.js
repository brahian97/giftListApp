...
export const GiftListCard = ({ giftList }) => {
  ...
  const { id, title, isPrivate, createdAt } = giftList
  const dispatch = useDispatch()

  const handleActivateGiftList = () => {
    dispatch(activeGiftList(id, giftList))
  }

  const handleDelete = () => {
    dispatch(deleteGiftList(id))
  }
  ...
  return (
    <div className='col'>
      <div className='card h-100 shadow'>
        ...
        <div className='card-body w-100' onClick={handleActivateGiftList}>
          <svg xmlns='http://www.w3.org/2000/svg' width='32' height='32' fill='currentColor' className='bi bi-gift' viewBox='0 0 16 16'>
            <path d='M3 2.5a2.5 2.5 0 0 1 5 0 2.5 2.5 0 0 1 5 0v.006c0 .07 0 .27-.038.494H15a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1v7.5a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 14.5V7a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h2.038A2.968 2.968 0 0 1 3 2.506V2.5zm1.068.5H7v-.5a1.5 1.5 0 1 0-3 0c0 .085.002.274.045.43a.522.522 0 0 0 .023.07zM9 3h2.932a.56.56 0 0 0 .023-.07c.043-.156.045-.345.045-.43a1.5 1.5 0 0 0-3 0V3zM1 4v2h6V4H1zm8 0v2h6V4H9zm5 3H9v8h4.5a.5.5 0 0 0 .5-.5V7zm-7 8V7H2v7.5a.5.5 0 0 0 .5.5H7z' />
          </svg>
          <h6 className='card-title'>{title}</h6>
        </div>
        <div className='card-footer text-muted'>
          <p style={{ marginBottom: '0' }}>Creada, <span>{parseGiftListDate(createdAt).toString()}</span></p>
          {isPrivate ? (<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 448 512' width='16' height='16' className='card-icon'>
            <path d='M144 144v48H304V144c0-44.2-35.8-80-80-80s-80 35.8-80 80zM80 192V144C80 64.5 144.5 0 224 0s144 64.5 144 144v48h16c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V256c0-35.3 28.7-64 64-64H80z' />
          </svg>) :
            (<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' width='16' height='16' className='card-icon'>
